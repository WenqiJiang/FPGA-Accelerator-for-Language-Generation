...
void wrapper_text_generation(
  ...
  // declare arrays
  FDATA_T* word_embedding_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * WORD_NUM * WORD_SIZE);
  FDATA_T *rnn_kernel_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * RNN_STATE_SIZE * RNN_INPUT_SIZE);
  FDATA_T *rnn_recurrent_kernel_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * RNN_STATE_SIZE * RNN_STATE_SIZE);
  FDATA_T *rnn_bias_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * RNN_STATE_SIZE);
  FDATA_T *fc_kernel_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * FC_OUTPUT_SIZE * FC_INPUT_SIZE);
  FDATA_T *fc_bias_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * FC_OUTPUT_SIZE);
...
// // #pragma HLS array_partition variable=fc_bias_BRAM cyclic factor=8
  ...
  FDATA_T *rnn_input_state_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * BATCH_SIZE * RNN_INPUT_SIZE);
  FDATA_T *rnn_state0_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * BATCH_SIZE * RNN_STATE_SIZE);
  FDATA_T *rnn_state1_BRAM = (FDATA_T*) MALLOC(sizeof(FDATA_T) * BATCH_SIZE * RNN_STATE_SIZE);
  IDATA_T *result_idx_one_step0 = (IDATA_T*) MALLOC(sizeof(IDATA_T) * BATCH_SIZE);
  IDATA_T *result_idx_one_step1 = (IDATA_T*) MALLOC(sizeof(IDATA_T) * BATCH_SIZE);
...
void wrapper_rnn_fc(
  ...
  FDATA_T *fc_output_feature_map = (FDATA_T*) MALLOC(sizeof(FDATA_T) * FC_OUTPUT_SIZE * BATCH_SIZE);
...
// #define DEBUG
  ...
  printf("\nrnn input state (of the first sample in the batch):\n");
    for (LDATA_T i = 0 ; i < RNN_STATE_SIZE; i++)
      printf("%f\t", TOFLOAT(rnn_input_state_cache[i]));
  printf("\n");

  printf("\nrnn output state (of the first sample in the batch):\n");
    for (LDATA_T i = 0 ; i < RNN_STATE_SIZE; i++)
      printf("%f\t", TOFLOAT(rnn_output_state[i]));
  printf("\n");

  printf("\nfc output state (of the first sample in the batch):\n");
    for (LDATA_T i = 0 ; i < RNN_STATE_SIZE; i++)
      printf("%f\t", TOFLOAT(fc_output_feature_map[i * BATCH_SIZE]));
  printf("\n");

  printf("result index one step:\n");
  for (LDATA_T i = 0; i < BATCH_SIZE; i++) {
    printf("%d\t", int(result_idx[i]));
  }
  printf("\n");
#endif
...
// #pragma HLS DATAFLOW
  ...
  printf("\nfirst batch fc output :\n");
    for (LDATA_T i = 0 ; i < BATCH_SIZE; i++)
      printf("%f\t", TOFLOAT(output_feature_map_reg[i]));
  printf ("\nkernel reg:\n");
  printf("\n");
  for (int i = 0; i < FC_INPUT_SIZE; i++) {
    printf("%f\t", TOFLOAT(kernel_reg[i]));
  }
  printf("\n");
    }
#endif
