...
// #define VERBOSE
    ...
    if (C_result[i] == Keras_result[i])
      ...
      printf("INFO: Sample %d:\t C_result: %d\t Keras_result: %d\t "
             "Actual_result: %d\t P(%d): %f\t P(%d): %lf\n",
             i, C_result[i], Keras_result[i], Actual_result[i],
             C_result[i], (softmax_result[i * SM_CLASS_SIZE + C_result[i]]),
             Keras_result[i],
             (softmax_result[i * SM_CLASS_SIZE + Keras_result[i]]));
    }
#endif
  }
