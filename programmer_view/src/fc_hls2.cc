...
void fc(DT input_feature_map[BATCH_SIZE * FC_INPUT_SIZE], 
        DT bias[FC_OUTPUT_SIZE], 
        DT kernel[FC_OUTPUT_SIZE * FC_INPUT_SIZE],
        DT output_feature_map[BATCH_SIZE * FC_OUTPUT_SIZE]);
...
void fc_save_output_feature_map(
    ...
    FDATA_T output_feature_map_BRAM[BATCH_SIZE * FC_OUTPUT_SIZE],
    LDATA_T start_batch);
...
void fc_save_output_feature_map(
  ...
  // output_feature_map_BRAM: BATCH_SIZE x FC_OUTPUT_SIZE
...
BATCH:
  for (LDATA_T batch_iter = 0; batch_iter < BATCH_SIZE / TILE_BATCH;
       batch_iter++) {
// #pragma HLS DATAFLOW
...
// #pragma SDS data zero_copy(input_feature_map[0: (BATCH_SIZE*FC_INPUT_SIZE)])
